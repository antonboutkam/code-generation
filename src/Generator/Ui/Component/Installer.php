<?php
namespace Generator\Ui\Component;
...
class Installer
{
     ...
     */
    static function create($aAnswers, OutputInterface $output)
    {
        ...
        foreach($aComponents as $oComponent)
        {
            ...
            foreach($oComponent->getComponentXml()->getProperties() as $oProperty)
            {
                if ($oProperty->getType() === 'enumeration')
                {
                    ...
                    $oLookupDataModelField = Property::toDataField($oLookupDataModel, $oProperty, ['field_name' => 'item_key', 'field_type' => 'string']);
                    $oLookupDataModelField->save();
                    $oLookupDataModelField = Property::toDataField($oLookupDataModel, $oProperty, ['field_name' => 'item_label', 'field_type' => 'string']);
                    $oLookupDataModelField->save();
                }
            }
